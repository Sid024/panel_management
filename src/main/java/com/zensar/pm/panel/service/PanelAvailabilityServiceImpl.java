...
//        timeWithId.addAll(timeSet);
        ...
        if ((pa.getToDate().isAfter(date1) || pa.getToDate().isEqual(date1)) && (pa.getToDate().isBefore(date2)||pa.getToDate().isEqual(date2)))
                {
...
//        }
	...
	public List<PanelAvailDTO> getPanelByFilterCriteria(String panelName, String token) {
		if (Constants.ROLE_PRACTICE_HEAD.equalsIgnoreCase(loginDelegate.isTokenValid(token).getRoleName())){
			CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();
	        CriteriaQuery<PanelAvailabilityEntity> criteriaQuery = criteriaBuilder.createQuery(PanelAvailabilityEntity.class);
	        ...
	        if (panelName != null && panelName.length()>=3 && !"".equals(panelName)) {
	            Predicate predicateLevelId = criteriaBuilder.like(root.get("panelsEntity").get("userEntity").get("userName"), '%'+panelName+'%');
	        ...
	        List<PanelAvailDTO> panelsDtoList = PanelAvailConstants.convertPanelAvailEntityListIntoPanelAvailDtoList( panelAvailList );
	...
	public PanelAvailDTO create(PanelAvailDTO pa, String token) {
		if (Constants.ROLE_PRACTICE_HEAD.equalsIgnoreCase(loginDelegate.isTokenValid(token).getRoleName())){
			...
			PanelAvailabilityStatusEntity panelAvailabilityStatusEntity = panelStatusRepo.findById(availabilityStatusId);
			int panelId = pa.getPanelId();
			PanelEntity panelsEntity = panelRepo.findById(panelId).get();
			PanelAvailabilityEntity panelAvailEntity=new PanelAvailabilityEntity(pa.getPanelsAvailabilityId(),panelsEntity,pa.getDate(),pa.getStartTime(),pa.getEndTime(),panelAvailabilityStatusEntity,pa.getCreatedBy(),pa.getCreatedOn(),pa.getUpdatedBy(),pa.getUpdatedOn(),pa.getDeletedBy(),pa.getDeletedOn(),pa.isDeleted());
			PanelAvailabilityEntity newPanelAvailEntity = repository.save(panelAvailEntity);
			PanelAvailDTO newPanelAvailDto = PanelAvailConstants.convertPanelAvailEntityIntoPanelAvailDto(newPanelAvailEntity);
